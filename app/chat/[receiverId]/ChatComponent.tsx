...
export default function ChatComponent({
  ...
  useEffect(() => {
    if (!isLoading) {
      const channel = pusherClient.subscribe(`chat-${senderId}-${receiverId}`)
          ...
          <div className="h-[500px] overflow-y-auto p-6 space-y-4 bg-gradient-to-b from-black/60 to-purple-900/20">
            {isLoading ? (
              ...
              <>
                {messages.length === 0 ? (
                  <div className="flex items-center justify-center h-full">
                    <p className="text-cyan-300 font-['Rajdhani'] text-xl">Secure channel established. Commence transmission.</p>
                  </div>
                ) : (
                  messages.map((message) => (
                    ...
                    >
                      ...
                      >
                        <p className="font-['Rajdhani'] text-lg">{message.content}</p>
